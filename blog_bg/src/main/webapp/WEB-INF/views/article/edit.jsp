      ...
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			...
			<div class="panel panel-default">
              <div class="panel-heading">修改文章<div style="float:right;cursor:pointer;"></div></div>
			  <div class="panel-body">
				<form role="form" id="formBtn" enctype="multipart/form-data">
				  		...
				  		<td>
				  			...
				  			<select name="categoryid" id="fcategorieid" style="width: 120px;margin-right: 10px;" class="form-control">
				  				<c:forEach items="${categories}" var="categorie">
				  					<option value="${categorie.id}" <c:if test="${article.categoryid eq categorie.id}">selected="selected"</c:if> >${categorie.name}</option>
				  				</c:forEach>
							</select>
							</div>
				  		</td>
				  ...
				  <button style="margin-top: 10px;" type="button" id="formSubmit" class="btn btn-success"><i class="glyphicon glyphicon-pencil"></i>发表文章</button>
				</form>
			  </div>
			</div>
        </div>
        ...
        <script type="text/javascript">
            $(function () {
			    $(".list-group-item").click(function(){
				    ...
				    var options = $("#fcategorieid option");
				    for(var i=0;i<options.length;i++){
				    	if(options[i].val()==categoryid){
			    ...
			    $("#formSubmit").click(function(){
			    	...
			    	$.ajax({
			    		url:"${APP_PATH}/article/update.do",
			    		type:"post",
			    		data:{
			    		...
			    		success:function(result){
			    			if(result.success){
			    				layer.msg("修改文章成功", {time:2000, icon:6},function(){
